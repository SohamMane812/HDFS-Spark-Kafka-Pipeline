...
object TaxiAnalysis {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder()
      ...
      .master("local[*]")  // Change or remove for cluster deployment
      .getOrCreate()

    val taxiDF = spark.read
      .option("header", "true")
      .option("inferSchema", "true")
      .csv("hdfs://localhost:9000/nyc_taxi/raw/cleaned_data.csv")
    ...
    taxiDF.write.mode("overwrite").partitionBy("VendorID")
      .parquet("hdfs://localhost:9000/nyc_taxi/parquet/")
